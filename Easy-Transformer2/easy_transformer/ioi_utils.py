    ...
    for (layer, head) in heads:
        ...
        for i, text in enumerate(prompts):
            ...
            if return_fig and not return_mtx:
                ...
                fig.show()
                fig.write_image(f"attention_pattern_L{layer}H{head}_prompt_{i}_{mode}.png")
...
def scatter_attention_and_contribution(
    ...
    for i, prompt in enumerate(ioi_dataset.ioi_prompts):
        ...
        io_tok = model.tokenizer(prompt["IO"])["input_ids"][1] # 0 is bos
        s_tok = model.tokenizer(prompt["S"])["input_ids"][1]
    ...
    if return_fig:
        ...
        fig.show()
        fig.write_image(f"attention_contribution_L{layer_no}H{head_no}_{suffix}.png")
