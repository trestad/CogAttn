...
def convert_my_weights(my, cfg: EasyTransformerConfig):
    state_dict = {}

    state_dict["embed.W_E"] = my['embedding.weight']

    for l in range(cfg.n_layers):
        state_dict[f"blocks.{l}.ln1.w"] = my[f'layers.{l}.attention_norm.weight']

        W_Q = my[f'layers.{l}.attention.qw']
        W_K = my[f'layers.{l}.attention.kw']
        W_V = my[f'layers.{l}.attention.vw']
        ...
        W_O = my[f'layers.{l}.attention.ow']
        W_O = einops.rearrange(W_O, "(i h) m->i h m", i=cfg.n_heads)
        state_dict[f"blocks.{l}.attn.W_O"] = W_O

        state_dict[f"blocks.{l}.ln2.w"] = my[f'layers.{l}.ffn_norm.weight']
       
        state_dict[f"blocks.{l}.mlp.gw"] = my[f'layers.{l}.ffn.shared_experts.0.gw']
        state_dict[f"blocks.{l}.mlp.pw"] = my[f'layers.{l}.ffn.shared_experts.0.pw']
        state_dict[f"blocks.{l}.mlp.W_out"] = my[f'layers.{l}.ffn.shared_experts.0.ow']
        
    state_dict[f"unembed.W_U"] = my['output.weight'].T
    
    state_dict["ln_final.w"] = my['norm.weight']
    return state_dict
