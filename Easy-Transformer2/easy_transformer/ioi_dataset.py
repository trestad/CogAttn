...
def get_end_idxs(prompts, tokenizer, name_tok_len=1, prepend_bos=False, toks=None):
    ...
    relevant_idx = 0 #int(prepend_bos)
...
class IOIDataset:
    def __init__(
        ...
        # for n in NAMES:
        #     input(self.tokenizer.tokenize(n))
            
        texts = [
            prompt["text"] for prompt in self.ioi_prompts
        ]
        # input(texts[0]) # Then, Matthew and Robert had a lot of fun at the school. Robert gave a ring to Matthew
        self.toks = torch.Tensor(self.tokenizer(texts, padding=True).input_ids).type(
            torch.int
        )
        # input(self.toks[0]) # tensor([    1,  1987, 29892, 22292,   322,  4755,   750,   263,  3287,   310,
        #  2090,   472,   278,  3762, 29889,  4755,  4846,   263,  9228,   304,
        # 22292,     2,     2,     2,     2,     2,     2,     2,     2,     2,
        #     2], dtype=torch.int32)
        ...
        # input(self.tokenizer.tokenize(texts[0]))
        # input(self.word_idx['IO'][0])
        # input(self.word_idx['S'][0])
        # input(self.word_idx['S2'][0])
        # input(self.word_idx['end'][0])
        ...
        self.io_tokenIDs = [
            toks[idx].item() for toks, idx in zip(self.toks, self.word_idx['IO'])
        ]

        self.s_tokenIDs = [
            toks[idx].item() for toks, idx in zip(self.toks, self.word_idx['S'])
        ]
        ...
        # input(self.tokenized_prompts[0])
