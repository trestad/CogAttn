    ...
    def from_pretrained(
        ...
        **model_kwargs,
        ...
        config = {
            ...
            "d_mlp": model_cfg['hidden_dim'],
            "n_ctx": model_cfg['max_seq_length'],
            "final_rms": True,
            "tokenizer_name": 'meta-llama/Llama-2-7b-chat-hf',
            "normalization_type": "foo",
            "act_fn": 'silu',
            "attn_types": ['vanilla'] + ['np'] * (model_cfg['n_layers'] - 2) + ['vanilla'] if 'cog' in model_name else ['vanilla'] * model_cfg['n_layers'],
        }

        model = cls(config, **model_kwargs)
        
        state_dict = weight_conversion.convert_my_weights(dict(state_dict["model"]), model.cfg)
